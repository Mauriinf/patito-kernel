 ...
 */
 int compare_zoj(const char *file1, const char *file2) {
 	...
 	f2 = fopen(file2, "r+");
 	if(DEBUG){
 		printf("Comparing %c with %c",*file1, *file2);
 	}
 	if (!f1 || !f2) {
 		ret = OJ_RE;
 	} else
...
void _get_solution_mysql(int solution_id, char * work_dir, int lang) {
	...
	MYSQL_RES *res;
	MYSQL_ROW row;
	sprintf(sql, "SELECT source FROM solution WHERE solution_id=%d",solution_id);
	mysql_real_query(conn, sql, strlen(sql));
...
void _get_problem_info_mysql(int p_id, int & time_lmt, int & mem_lmt, int & isspj) {
	...
	MYSQL_RES *res;
	MYSQL_ROW row;
	sprintf(sql,"SELECT time_limit,memory_limit FROM problem where problem_id=%d", p_id);
	mysql_real_query(conn, sql, strlen(sql));
	res = mysql_store_result(conn);
	row = mysql_fetch_row(res);
	time_lmt = atoi(row[0]);
...
//jvm gc ask VM before need,so used kernel page fault times and page size
		...
		if (WIFEXITED(status))
			break;
		if ( lang < 4 && get_file_size("error.out") && !oi_mode) {
			if (DEBUG)
	...
	if (WIFSIGNALED(status)) {
  	 ...
  	 if (call_counter[reg.REG_SYSCALL] ){
			...
			sprintf(error, "[ERROR] A Not allowed system call: runid:%d callid:%ld\n TO FIX THIS , ask admin to add the CALLID into corresponding LANG_XXV[] located at okcalls32/64.h ,and recompile judge_client", solution_id, (long)reg.REG_SYSCALL);
			write_log(error);
			if (DEBUG)
				printf("RE  %s\n", error);
...
int get_sim(int solution_id, int lang, int pid, int &sim_s_id) {
	...
	if (!sim) {
		...
		//c cpp will
		if (lang == 0)
			execute_cmd("/bin/ln ../data/%d/ac/%d.%s ../data/%d/ac/%d.%s", pid, solution_id, lang_ext[lang], pid, solution_id,
				lang_ext[lang + 1]);
		if (lang == 1)
			execute_cmd("/bin/ln ../data/%d/ac/%d.%s ../data/%d/ac/%d.%s", pid, solution_id, lang_ext[lang], pid, solution_id,
				lang_ext[lang - 1]);
